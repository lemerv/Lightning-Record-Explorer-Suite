...
import HierarchyCardField from "c/lresHierarchyCardField";

describe("c-lres-hierarchy-card-field", () => {
  afterEach(() => {
  ...
  const buildComponent = (props = {}) => {
    const element = createElement("c-lres-hierarchy-card-field", {
      is: HierarchyCardField
    });
  ...
  it("renders label and value when not a parent badge", () => {
    ...
    const label = element.shadowRoot.querySelector(
      ".hierarchy-card_field-label"
    );
    const value = element.shadowRoot.querySelector(
      ".hierarchy-card_field-value"
    );
